...
<xsl:stylesheet version="2.0" xmlns:fo="http://www.w3.org/1999/XSL/Format"
		...
		<fo:page-sequence master-reference="Main" initial-page-number="1">
			...
			<fo:flow flow-name="xsl-region-body">
				<fo:block>
					...
					<fo:marker marker-class-name="endWarning">
	...
	<xsl:template match="note">
		<fo:block xsl:use-attribute-sets="normal-text" keep-with-previous.within-page="always" keep-together.within-page="always">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="ancestor::levelledPara or ancestor::sbTopicContent">
				...
				<xsl:when test="count(ancestor::proceduralStep)=5">
					<xsl:attribute name="start-indent">1.8in</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="caution">
		<fo:block  xsl:use-attribute-sets="normal-text" keep-with-next.within-page="always" keep-together.within-page="always">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:choose>
	...
	<xsl:template match="warning">
		<fo:block xsl:use-attribute-sets="normal-text" keep-together.within-page="always" keep-with-next.within-page="always">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:choose>
	...
	<xsl:template match="sbAccomplishmentInstructions">
		<fo:block font-weight="bold" font-family="arial" text-align="left" keep-with-next.within-page="always" padding-after=".1in" id="{'accomplish-instruc'}">
			<fo:list-block provisional-distance-between-starts=".3in">
				<fo:list-item>
					<fo:list-item-label end-indent="label-end()">
					<fo:block><xsl:number count="sbPlanningInfo|sbMaterialInfo|sbAccomplishmentInstructions|sbAdditionalInfo" from="sb" level="single" format="1."/></fo:block>
	...
	<xsl:template match="levelledPara[count(ancestor::levelledPara)=0]">
		<fo:block start-indent=".3in" xsl:use-attribute-sets="normal-text">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:if test="@id">
	...
	<xsl:template match="para[parent::sbTopicContent][not(preceding-sibling::*)]">
		<fo:block font-weight="normal" start-indent=".6in">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:if test="ancestor::sbRevisionInfo or ancestor::sbSummary">
	...
	<xsl:template match="proceduralStep[count(ancestor::proceduralStep)=0]">
		<fo:block start-indent=".3in" font-weight="bold" font-family="arial">
			<xsl:if test="following-sibling::*">
				<xsl:attribute name="padding-after">.1in</xsl:attribute>
			</xsl:if>
			<xsl:if test="@id">
	...
	<xsl:template match="sbMaterialInfo">
		<fo:block font-weight="bold" font-family="arial" text-align="left" keep-with-next.within-page="always" padding-after=".1in" id="{'material-info'}">
			<fo:list-block provisional-distance-between-starts=".3in">
				<fo:list-item>
					<fo:list-item-label end-indent="label-end()">
					<fo:block><xsl:number count="sbPlanningInfo|sbMaterialInfo|sbAccomplishmentInstructions|sbAdditionalInfo" from="sb" level="single" format="1."/></fo:block>
	...
	<xsl:template match="sbSuppEquipsList">
		<fo:block xsl:use-attribute-sets="normal-text" font-weight="bold" text-align="left" padding-after=".1in" keep-with-next.within-page="always">
			
			<fo:list-block provisional-distance-between-starts=".3in">
						<fo:list-item>
							<fo:list-item-label end-indent="label-end()">
							<fo:block>
								<xsl:number count="sbTopic|sbSuppEquipsList" format="A." level="single"/>
							</fo:block>
	...
	<xsl:template match="sbSupportEquipsList">
		<fo:block start-indent=".3in" xsl:use-attribute-sets="normal-text" font-weight="bold" text-align="left" padding-after=".1in" keep-with-next.within-page="always">
			
			<fo:list-block provisional-distance-between-starts=".3in">
						<fo:list-item>
							<fo:list-item-label end-indent="label-end()">
							<fo:block>
								<xsl:number count="sbTopic|sbSupportEquipsList" format="A." level="single"/>
							</fo:block>
	...
	<fo:block>
		<fo:marker marker-class-name="sbFooter">
		<fo:block text-align="center" linefeed-treatment="preserve">
			<xsl:value-of select="ext:getSbFooter('dave')"/>		
				</fo:block>
	...
	<xsl:template match="sbPlanningInfo">
		<fo:block font-weight="bold" font-family="arial" text-align="left" keep-with-next.within-page="always" padding-after=".1in" id="{'planning-info'}">
			<fo:list-block provisional-distance-between-starts=".3in">
				<fo:list-item>
					<fo:list-item-label end-indent="label-end()">
					<fo:block><xsl:number count="sbPlanningInfo|sbMaterialInfo|sbAccomplishmentInstructions|sbAdditionalInfo" from="sb" level="single" format="1."/></fo:block>
	...
	<xsl:template match="sbAdditionalInfo">
		<fo:block font-weight="bold" font-family="arial" text-align="left" keep-with-next.within-page="always" padding-after=".1in" id="{'additional-info'}">
			<fo:list-block provisional-distance-between-starts=".3in">
				<fo:list-item>
					<fo:list-item-label end-indent="label-end()">
					<fo:block><xsl:number count="sbPlanningInfo|sbMaterialInfo|sbAccomplishmentInstructions|sbAdditionalInfo" from="sb" level="single" format="1."/></fo:block>
	...
	<xsl:template match="sbTopic">
		<fo:block font-weight="bold" font-family="arial" text-align="left" padding-after=".1in" id="{@id}">
		<xsl:if test="./title">
			<xsl:choose>
				<xsl:when test="parent::sbRevisionInfo">
				...
				<xsl:otherwise>
					...
					<fo:list-block provisional-distance-between-starts=".3in" padding-after=".1in">
						<fo:list-item>
							<fo:list-item-label end-indent="label-end()">
							<fo:block>
								<xsl:number count="sbTopic|sbSuppEquipsList" format="A." level="single"/>
	...
	<xsl:template match="sbTopicContent">
		<fo:block start-indent=".6in" font-family="arial" >
			<xsl:if test="ancestor::sbRevisionInfo or ancestor::sbSummary">
				<xsl:attribute name="start-indent">0</xsl:attribute>
			</xsl:if>
			<xsl:if test="following-sibling::*">
	...
	<xsl:template name="title-page">
		<fo:block xsl:use-attribute-sets="normal-text" text-align="center" break-after="page" id="{'title-page'}">
			...
			<fo:block xsl:use-attribute-sets="bold-normal-text" font-size="10pt" padding-after=".1in">
				<xsl:apply-templates select="descendant::simplePara[1]"/>
			</fo:block>
			<fo:block xsl:use-attribute-sets="bold-normal-text" font-size="14pt" padding-after=".1in">
	...
	<xsl:template name="static-content">
		<fo:static-content flow-name="Main.Header">
			<fo:block>
				<fo:block font-family="arial" font-size="10pt" font-weight="bold" text-align="center" padding-after=".1in">
					<fo:external-graphic content-height="scale-to-fit" content-width="3in" src="C:/inetpub/wwwroot/Projects/SB/Airbus/UTAS_header_logo_small_black.cgm"/><fo:block/>
		...
		<fo:static-content flow-name="Main.Footer">
			<fo:block>
				...
				<fo:table>
					<fo:table-column column-number="1" column-width="2.33in"/>
					<fo:table-column column-number="2" column-width="2.33in"/>
					<fo:table-column column-number="3" column-width="2.33in"/>
					<fo:table-body>
						<fo:table-row>
							<fo:table-cell number-columns-spanned="3">
								<fo:block xsl:use-attribute-sets="bold-normal-text" text-align-last="justify">
						...
						<fo:table-row>
							<fo:table-cell padding-before=".1in" display-align="before">
								<fo:block xsl:use-attribute-sets="normal-text">
				...
				<fo:retrieve-marker retrieve-class-name="sbFooter"/>
				<!--
				<fo:retrieve-marker retrieve-class-name="endWarning"></fo:retrieve-marker>-->
			</fo:block>
		</fo:static-content>
	</xsl:template>
