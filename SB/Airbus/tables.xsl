  ...
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="table">
		<xsl:if test="descendant::tbody/descendant::entry">
			<fo:block space-after=".15in" hyphenate="false" wrap-option="wrap" id="{generate-id(.)}" start-indent="0">
				<xsl:if test="@id">
					<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
				</xsl:if>
				<fo:table hyphenate="false" wrap-option="wrap" border-bottom-style="solid" border-bottom-width="1pt">
	...
	<xsl:template match="entry">
		<fo:table-cell wrap-option="wrap" hyphenate="false">
			...
			<fo:block xsl:use-attribute-sets="normal-text" start-indent="0" hyphenate="false" wrap-option="wrap">
				<xsl:if test="ancestor::thead">
					<xsl:attribute name="font-weight">bold</xsl:attribute>
				</xsl:if>
