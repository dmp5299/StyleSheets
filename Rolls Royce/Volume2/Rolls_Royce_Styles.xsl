...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="pnr">
		...
		<xsl:value-of select="concat('Pt. No.: ',./text())"/>
	</xsl:template>

	<xsl:template match="mfc">
		<fo:block padding-before="3pt"/>
		<xsl:value-of select="concat('NSCM: ',./text())"/>
	</xsl:template>
						...
						<fo:table-row>
							<fo:table-cell >
								<fo:block border-bottom-style="solid" border-bottom-width="1pt">
									<fo:external-graphic src="C:/inetpub/wwwroot/Projects/Rolls Royce/Volume2/logo.jpg" content-height=".44in" margin="none"/>
								</fo:block>
							</fo:table-cell>
						</fo:table-row>
					</fo:table-body>
...
<xsl:template match="step1/para|step2/para|step3/para">
	...
	<xsl:template match="graphic">
		<fo:block vertical-align="middle" text-align="center" width="7in" padding-after=".15in" start-indent="0">
			<!--<xsl:choose>
				<xsl:when test="ancestor::ipc">
					<xsl:attribute name="break-after">page</xsl:attribute>
					<fo:external-graphic content-height="8in" content-width="7in" vertical-align="middle">
						<xsl:attribute name="src">
							<xsl:choose>
								<xsl:when test="contains(@boardno,'.jpg')">
									<xsl:value-of select="@boardno"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat(@boardno,'.cgm')"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
					</fo:external-graphic>
				</xsl:when>
				<xsl:when test="@reproductionHeight and @reproductionWidth">
					<fo:external-graphic content-height="scale-down-to-fit" content-width="scale-down-to-fit" scaling="uniform" >
				...
				<xsl:when test="@reproductionHeight">
					<fo:external-graphic content-height="scale-down-to-fit" content-width="7in" scaling="uniform" >
						<xsl:attribute name="height">
							<xsl:value-of select="@reproductionHeight"/>
						</xsl:attribute>
						<xsl:attribute name="src">
				...
				<xsl:otherwise>
					<fo:table width="100%" start-indent="0">
						<fo:table-body>
							<fo:table-row>
								<fo:table-cell start-indent="0">
									<fo:block text-align="center">
										<fo:external-graphic content-height="scale-down-to-fit" content-width="7in" scaling="uniform" width="100%" >
											<xsl:attribute name="src">
												<xsl:choose>
													<xsl:when test="contains(@boardno,'.jpg')">
														<xsl:value-of select="@boardno"/>
			...
			<xsl:choose>
                        <xsl:when test="ext:fileExists(concat(@boardno,'.cgm'))" >
                                <fo:external-graphic  content-height="210mm" scaling="uniform" content-width="7in">
                                        <xsl:attribute name="src" >
                                                <xsl:value-of select="concat('C:/inetpub/wwwroot/Graphics/',@boardno,'.cgm')"/>
                                        </xsl:attribute>
                                </fo:external-graphic>
                        </xsl:when>
			...
			<xsl:choose>
				<xsl:when test="count(../graphic) &gt; 1">
					<fo:block xsl:use-attribute-sets="normal-text" font-style="italic"  keep-with-previous.within-page="always">
						...
						<fo:leader leader-length=".06in"/>
						(Sheet<xsl:value-of select="concat(' ',preceding-sibling::*[1][self::sheet]/@sheetno,' ')"/>
