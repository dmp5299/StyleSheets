...
<fo:change-bar-begin change-bar-class="{generate-id(.)}" change-bar-style="solid"/>
			...
			<fo:block space-after=".15in" hyphenate="true" wrap-option="wrap" id="{generate-id(.)}" start-indent="0">
				<fo:table hyphenate="true" wrap-option="wrap" border-bottom-style="solid" border-bottom-width="1pt" border-after-width.conditionality='retain'>
					<xsl:if test="ancestor::dmodule/@infoCode='014'">
						<xsl:attribute name="break-before">page</xsl:attribute>
					</xsl:if>
					<xsl:choose>
						<xsl:when test="ancestor::dmodule/@infoCode='014'">
							<fo:marker marker-class-name="generatedModuleTitle">
								<xsl:value-of select="'Equipment Designator'"/>
							</fo:marker>
						</xsl:when><!--
						<xsl:when test="ancestor::dmodule/@infoCode='013'">
							<fo:marker marker-class-name="generatedModuleTitle">
								<xsl:value-of select="'Numerical Index'"/>
							</fo:marker>
						</xsl:when>-->
					</xsl:choose>
										...
										<xsl:variable name="width">
											<xsl:choose>
												<xsl:when test="ancestor::foldout">
													<xsl:value-of select="13 div $count" />
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="6.98 div $count" />
												</xsl:otherwise>
											</xsl:choose>
										</xsl:variable>
					...
					<xsl:choose>
						<xsl:when test="ancestor::dmodule/@infoCode='014'">
							<fo:table-header>
								<fo:table-row>
									<fo:table-cell number-columns-spanned="{descendant::tgroup/@cols}" >
						...
						<xsl:when test="./title and not(ancestor::dmodule/descendant::dmCode[@infoCode='013']) and (not(ancestor::dmodule/descendant::dmCode[@infoCode='013']))">
							<fo:table-header>
								<fo:table-row>
									<fo:table-cell number-columns-spanned="{descendant::tgroup/@cols}" padding-after="3pt">
										<xsl:attribute name="border-bottom-style">solid</xsl:attribute>
										<xsl:attribute name="border-bottom-width">1pt</xsl:attribute>
										<xsl:apply-templates select="./title"/>
									</fo:table-cell>
								...
								<fo:table-row keep-with-next.within-page="always">
									<fo:table-cell number-columns-spanned="{descendant::tgroup/@cols}" padding-after="3pt">
										...
										<fo:block font-style="italic" xsl:use-attribute-sets="normal-text" text-align="center"><!--
											...
											<xsl:value-of select="$count+1"/><fo:retrieve-table-marker retrieve-class-name="cont"/>-->
										</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<xsl:apply-templates select="./tgroup/thead"/>
	...
	<xsl:template match="table/title">
		<fo:block xsl:use-attribute-sets="normal-text" text-align="center" padding-after="2pt" font-style="italic">
			...
			<xsl:apply-templates/><fo:retrieve-table-marker retrieve-class-name="cont"/>
		</fo:block>
	</xsl:template>
