							...
							<xsl:when test="./pmEntry">
								...
								<fo:bookmark internal-destination="{generate-id(.)}" starting-state="hide">
									...
									<xsl:for-each select="./dmodule[not(descendant::infoName='Change record or highlights')]">
										<fo:bookmark internal-destination="{generate-id(.)}" starting-state="hide">
													<fo:bookmark-title>
														<xsl:value-of select="descendant::techName[1]"/>
														<xsl:value-of select="' - '"/>
															<xsl:value-of select="descendant::infoName[1]"/>
													</fo:bookmark-title>
												</fo:bookmark>
									</xsl:for-each>
									<xsl:for-each select="./pmEntry">
									...
									<xsl:if test="not(@infoCode = '001')">
										<fo:bookmark internal-destination="{generate-id(.)}" starting-state="hide">
											<fo:bookmark-title>
												...
												<xsl:value-of select="descendant::techName[1]"/><xsl:value-of select="' - '"/><xsl:value-of select="descendant::infoName[1]"/>
											</fo:bookmark-title>
										</fo:bookmark>
									</xsl:if>
								</xsl:for-each>
